...
class Bin:
    def __init__(self, bin_config, index, rack_id, bin_manager, server_ip,kt_id):
    ...
    def send_message(self, bin_index, operation):
        """Send a message to the server with bin details."""
        ...
        try:
            # Make the request based on the prepared request data
            response = urequests.request(request_data['method'], request_data['url'], json=request_data['data'])
            if response.status_code == 200:
                print("Message sent successfully.")
            else:
                print(f"Failed to send message: {response.status_code}. Adding to queue.")
                add_to_queue(request_data)  # Add to queue if the request fails
